...
for i = 1:dof
    jnt_axs_k = str2num(kuka.robot.joint{i+1}.axis.Attributes.xyz)';
    % Transformation from parent link frame p to current joint frame
    rpy_k = sym(str2num(kuka.robot.joint{i+1}.origin.Attributes.rpy));
    R_pj = RPY(rpy_k);
    R_pj(abs(R_pj)<sqrt(eps)) = sym(0); % to avoid numerical errors
    p_pj = str2num(kuka.robot.joint{i+1}.origin.Attributes.xyz)';
